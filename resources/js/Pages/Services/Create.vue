...
<div class="bg-white shadow overflow-hidden sm:rounded-lg">
  ...
  <div class="px-4 py-5 sm:px-6">
    <h3 class="text-lg leading-6 font-medium text-gray-900">Information du Service</h3>
    ...
    <dl>
      <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 items-center sm:px-6">
        ...
        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
             <BreezeInput   type="text" v-model="form.cod_ser" class="block w-full mt-1"    required  autofocus   />
      ...
      <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 items-center sm:px-6">
        ...
        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
             <BreezeInput   type="text" v-model="form.libelle_ser" class="block w-full mt-1"    required  autofocus   />
...
const submit = () => {
    ...
    form.cod_ser = form.cod_ser.toUpperCase()
    form.post(route('services.store'), {
        onFinish: () => form.reset('cod_ser'),
    })
};

const passIsValid = computed (() => {
    if(!form.cod_ser || form.cod_ser.length < 4) {
        return false
    }
    props.services.forEach(element => {
        if(element == form.cod_ser.toUpperCase()) {
            return false
        }
    });
  return true;
