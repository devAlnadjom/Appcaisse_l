            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <form @submit.prevent="submit">
                    <div class="px-4 py-5 sm:px-6">
                        ...
                        <p class="mt-1 max-w-2xl text-sm text-gray-500">Veuillez remplir convenablement le formulaire.
                        </p>
                        <div class="mt-4">
                        </div>
                    </div>
                        ...
                        <dl>
                            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 items-center sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <BreezeInput id="receveur_pdc" type="text" v-model="form.receveur_pdc"
                            ...
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm font-semibold text-gray-900 sm:mt-0 sm:col-span-2">
                                    <BreezeInput id="montant_pdc" type="number" v-model="form.montant_pdc"
                                        class="block w-full mt-1" required autofocus />
                                    ...
                                    <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                        <li v-if="!isManager"
                                            ...
                                            <div class="ml-4 flex-shrink-0">
                                                <select v-model="form.team_l"
                                                    class=" text-sm px-2  border border-gray-300 rounded-md shadow-sm  focus:border-indigo-400  focus:ring-1 focus:ring-indigo-400 focus:ring-opacity-50 focus-within:text-indigo-900 p-1">
                                                    <option :value="null">Choisissez Votre Manager
                                                        &nbsp;&nbsp;&nbsp;&nbsp;</option>
                                                    <option v-for="manager in managers " :key="manager.id_ui"
                                                        :value="manager.id_ui">{{ manager.nom_p_ui }}</option>
                                                </select>
                                            </div>
                                        </li>
                                        <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                            ...
                                            <div class="ml-4 flex-shrink-0">
                                                <select v-model="form.projet_pdc"
                                                    class="text-sm px-2  border border-gray-300 rounded-md shadow-sm  focus:border-indigo-400  focus:ring-1 focus:ring-indigo-400 focus:ring-opacity-50 focus-within:text-indigo-900 p-1">
                                                    <option :value="null">Choisir un Projet</option>
                                                    <option v-for="projet in projets " :key="projet.id_projet"
                                                        :value="projet.id_projet">{{ projet.libelle_projet }}</option>
                                                </select>
                                            </div>
                                        </li>
                                        <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                            ...
                                            <div class="ml-3 flex-shrink-0 ">
                                                ...
                                                <select v-model="form.code_service_pdc"
                                                    class="text-sm px-2  border border-gray-300 rounded-md shadow-sm  focus:border-indigo-400  focus:ring-1 focus:ring-indigo-400 focus:ring-opacity-50 focus-within:text-indigo-900 p-1">
                                                    <option :value="null">Choisir un Service</option>
                                                    <option v-for="service in services " :key="service.cod_ser"
                                                        :value="service.cod_ser">{{ service.libelle_ser }}
                                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</option>
                                                </select>
                                        ...
                                        <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                            ...
                                            <div class="ml-4 flex-shrink-0">
                                                <select v-model="form.data3_pdc"
                                                    class=" text-sm px-2 border border-gray-300 rounded-md shadow-sm  focus:border-indigo-400  focus:ring-1 focus:ring-indigo-400 focus:ring-opacity-50 focus-within:text-indigo-900 p-1">
                                                    <option :value="0">Choisir un Bon</option>
                                                    <option v-for="commande in commandes " :key="commande.id_bc"
                                                        :value="commande.id_bc"> {{commande.numero_bc + " -"+commande.nom_cli_bc}}</option>
                                                </select>
                                                <p class="text-xs text text-indigo-700 mt-2"> Ne Choisissez que si vous
                                                    êtes sur.</p>
                                            </div>
                                        </li>
                                    </ul>
                            ...
                            <div class="bg-white border-t px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <div class="flex items-center justify-end mt-4">
                                        ...
                                        <BreezeButton class="ml-8" :class="{ 'opacity-25': form.processing }"
